...
class WikipediaEditEventIrcStream {
	...
	private static final Logger LOG = LoggerFactory.getLogger(berlin.bbdc.inet.wordcountMetrics.WikipediaEditEventIrcStream.class);
	...
	private static class WikipediaIrcChannelListener  extends RichMapFunction<String,Long> implements IRCEventListener {
		...
		public void open(Configuration config){
			this.counter = getRuntimeContext().getMetricGroup().addGroup("WikiGroup").counter("WikiIRCChannelListenerMetric");
		}
		...
		public void onPrivmsg(String target, IRCUser user, String msg) {
			...
			try {
				LOG.debug("Registered Counter Metric");
				map(user.getNick());
			} catch (Exception e) {
				e.printStackTrace();
			}
